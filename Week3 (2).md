...
```md
결측치가 있는 컬럼: 결측치가 있는 컬럼이 없었다
사용한 처리 방법: 결측치가 없어 별도 처리를 안했다
그 방법을 선택한 이유: 결측치가 없어 별도 처리를 안했다
```
...
```md
확인한 숫자 컬럼: MonthlyIncome
이상해 보이는 값: max값이 19999로 평균(6503)보다 훨씬 높지만, 상위 10개 값을 직접 확인한 결과 경력과 부서가 납득이 가 실제 고소득자의 데이터로 판단했다. 
처리 여부와 이유: 별도로 처리하지 않음
```
...
```
```md
숫자 변수는 전부 int 64로 제대로 읽혔으며, 날짜 컬럼은 존재하지 않는다.
```
---
